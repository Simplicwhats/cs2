...
let recoilOffset = new THREE.Vector2(0, 0);
let activeGrenades = [];
...
function createNetworkPlayer(id, nick) {
    ...
    const rifle = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.12, 0.7), matGun);
    rifle.position.set(0.2, 1.05, -0.25); rifle.rotation.x = 0.2;
...
function build3DWeapon() {
    ...
    const mDark = new THREE.MeshStandardMaterial({ color: 0x1f1f1f, metalness: 0.8, roughness: 0.3 });
    ...
    if (activeSlot === 'grenade') {
        const nadeMesh = new THREE.Mesh(new THREE.SphereGeometry(0.06, 16, 16), mGreen);
        ...
        const body = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.09, 0.6), mGreen);
        const scope = new THREE.Mesh(new THREE.CylinderGeometry(0.018, 0.018, 0.25), mDark); scope.rotation.x = Math.PI/2; scope.position.set(0, 0.07, -0.05);
...
function setupEvents() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if (e.code === 'KeyW' && !moveF) {
            ...
            if (now - lastShotTime < 250) isRunning = true;
            moveF = true;
        }
